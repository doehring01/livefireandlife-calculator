...
(function () {
  ...
  function openGate() {
    ...
    if (gate && gate.style) { gate.style.display = "flex"; return true; }
    return false;
  }
  function closeGate() {
    var gate = $("nlBackdropCF");
  ...
  function handleUnlockClick(e) {
    ...
    // already unlocked? show immediately
    if (isUnlocked()) {
    ...
    // open modal if present, else hard-redirect
    if (!openGate()) {
  ...
  function init() {
    ...
    if (unlockBtn) {
      // use capture=false and only one live listener
      unlockBtn.addEventListener("click", handleUnlockClick, false);
    }

    // Modal dismiss by clicking the backdrop (outside card)
    ...
    if (nlForm) {
      nlForm.addEventListener("submit", function () {
        // Let Mailchimp open in a new tab; unlock immediately
        unlock();
        setTimeout(function () {
          closeGate();
          if (!lastInputs) { lastInputs = readInputs(); lastCalc = computeCoast(lastInputs); }
    ...
    if (nlSkip) {
      nlSkip.addEventListener("click", function () {
        unlock(); // treat skip as soft unlock
        closeGate();
        if (!lastInputs) { lastInputs = readInputs(); lastCalc = computeCoast(lastInputs); }
        renderFullResults(lastInputs, lastCalc);
        show($("unlockBlock"), false);
